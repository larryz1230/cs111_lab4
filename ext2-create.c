...
void write_block_bitmap(int fd)
{
	...
	// Set the first 24 blocks as occupied
	...
	// Set all bits after the first 1024 bits as occupied
	...
	if (off == -1)
	{
		errno_exit("lseek block bitmap");
	}
	if (write(fd, &map_value, NUM_BLOCKS) != NUM_BLOCKS)
	{
		errno_exit("write block bitmap");
	}
}
...
void write_inode_table(int fd) {
	...
	//build struct for symlink inode
	...
	hello_inode.i_size = 11;			//size of "hello-world" string
...
void write_root_dir_block(int fd)
{
	...
	//parent entry is the same as current directory for root dir
...
void write_hello_world_file_block(int fd)
{
	...
	int bytes_written = write(fd, "Hello world\n", 12);
	if (bytes_written != 12)		
	{
		errno_exit("write hello_world failure");
	}
}
